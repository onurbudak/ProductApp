...
public class UpdateUserCommandHandler : ICommandHandler<UpdateUserCommand, User>
{
    ...
    public async Task<ServiceResponse<User>> Handle(UpdateUserCommand request, CancellationToken cancellationToken)
    {
        ...
        if (user is null)
        {
            return ServiceResponse<User>.FailureDataWithMessage(Messages.RecordIsNotFound, new Error(MessageCode.RecordIsNotFound, Messages.RecordIsNotFound));
        }
